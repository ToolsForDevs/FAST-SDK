package com.toolsfordevs.fast.core.ui.seekbar;
...
public class TickSeekBar extends View {
    ...
    public TickSeekBar(Builder builder) {
        ...
        int defaultPadding = Dimens.dp(16);
    ...
    private void initParams() {
        ...
        if (mThumbDrawable == null) {
            ...
            mThumbRadius = Math.min(Dimens.dp(THUMB_MAX_WIDTH), mThumbSize) / 2.0f;
            mThumbTouchRadius = mThumbRadius;
        }
        if (mTickMarksDrawable == null) {
            mTickRadius = mTickMarksSize / 2.0f;
        } else {
            mTickRadius = Math.min(Dimens.dp(THUMB_MAX_WIDTH), mTickMarksSize) / 2.0f;
        }
    ...
    private void initDefaultPadding() {
        if (!mClearPadding) {
            int normalPadding = Dimens.dp(16);
            if (getPaddingLeft() == 0) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isAboveBelowText()) {
            setMeasuredDimension(resolveSize(Dimens.dp(170), widthMeasureSpec), height + 2 * mTickTextsHeight);
        } else {
            setMeasuredDimension(resolveSize(Dimens.dp(170), widthMeasureSpec), height + mTickTextsHeight);
        }
        initSeekBarInfo();
...
//        initTickXAndTextXLocation();
    ...
    private void initTrackLocation() {
        if (mR2L) {
            ...
            if (hasAboveText()) {
                mBackgroundTrack.top = mPaddingTop + mThumbTouchRadius + mDefaultTickTextsHeight + Dimens.dp(3);
            } else {
            ...
            if (hasAboveText()) {
                mProgressTrack.top = mPaddingTop + mThumbTouchRadius + mDefaultTickTextsHeight + Dimens.dp(3);
            } else {
    ...
    private Bitmap getDrawBitmap(Drawable drawable, boolean isThumb) {
        ...
        int width;
        int height;
        int maxRange = Dimens.dp(THUMB_MAX_WIDTH);
    ...
    private void measureTickTextsBonds() {
        if (needDrawText()) {
            ...
            mTickTextsHeight = mRect.height() + Dimens.dp(3);//with the gap(3dp) between tickTexts and track.
        }
    }
        ...
        if (needDrawText()) {
            ...
            if (isAboveBelowText()) {//one of the text is below seek bar, the other is above.
                if (mTickTextsPosition == TextPosition.BELOW) {
                    mThumbTextY = mPaddingTop + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + Dimens.dp(3);
                    mTickTextY = mTickTextsHeight + mPaddingTop + mCustomDrawableMaxHeight + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + Dimens.dp(3);
                } else {
                    mTickTextY = mPaddingTop + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + Dimens.dp(3);
                    mThumbTextY = mTickTextsHeight + mPaddingTop + mCustomDrawableMaxHeight + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + Dimens.dp(3);
                }
                return;
            }
            if (hasBelowText()) {//both text below seek bar
                mTickTextY = mPaddingTop + mCustomDrawableMaxHeight + Math.round(mDefaultTickTextsHeight - mTextPaint.descent()) + Dimens.dp(3);
