...
internal class ReceiversView(context: Context) : FrameLayout(context)
{
    ...
    init
    {
        ...
        val packageInfo = AppInstance.get().packageManager.getPackageInfo(AppInstance.get().packageName, PackageManager.GET_RECEIVERS)
        packageInfo.receivers?.filter { !it.name.startsWith("com.toolsfordevs.fast") }?.forEach { addActivity(it) }
    ...
    private fun addActivity(activityInfo: ActivityInfo)
    {
        ...
        list.add(KeyValue("descriptionRes", ResUtils.getStringResourceName(activityInfo.descriptionRes)))
